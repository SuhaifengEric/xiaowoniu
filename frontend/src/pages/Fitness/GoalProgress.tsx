...
export default function GoalProgress({ goal, stats, loading, onEdit }: GoalProgressProps) {
  ...
  if (!goal) return (
    <section className="fitness-panel flex flex-col items-start justify-between gap-5 sm:flex-row sm:items-center" aria-labelledby="goal-empty-title">
      ...
      <Button type="button" onClick={onEdit} className="min-h-11 bg-emerald-800 hover:bg-emerald-900">创建目标</Button>
    </section>
  )
  ...
  return (
    <section className="fitness-panel" aria-labelledby="goal-title">
      ...
      <Progress value={progress} aria-label="本周运动目标完成度" className="mt-5 bg-stone-200 [&>div]:bg-emerald-700" />
      <p className={`mt-2 text-sm font-medium ${completed >= target && target > 0 ? 'text-emerald-800' : 'text-stone-600'}`}>{status}</p>
