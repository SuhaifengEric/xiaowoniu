...
const dashboardActions: Record<string, LearningDialogName> = {
...
export default function Learning() {
  ...
  useEffect(() => {
    ...
    const dialogName = action ? dashboardActions[action] : undefined
    if (!action) return
    if (dialogName === 'subject-create' && !selectedExam) return
    if (dialogName === 'checkin' && subjects.length === 0) return

    const nextParams = new URLSearchParams(searchParams)
    nextParams.delete('action')
    setSearchParams(nextParams, { replace: true })

    if (dialogName === 'exam-create') openExamCreate(action)
    if (dialogName === 'subject-create') openSubjectCreate(action)
    if (dialogName === 'checkin') openCheckin(formatLocalDate(new Date()), action)
  }, [searchParams, selectedExam, setSearchParams, subjects.length])
  ...
  const openCheckin = (date = formatLocalDate(new Date()), dashboardAction: string | null = null) => {
  ...
  const openExamCreate = (dashboardAction: string | null = null) => {
    setDashboardReturnFocus(dashboardAction)
    setEditingExam(null)
    setStatus('')
    setDialog('exam-create')
  }

  const openSubjectCreate = (dashboardAction: string | null = null) => {
    ...
    <main className="app-page learning-page has-mobile-tabbar" data-dialog-return-focus={dashboardReturnFocus ?? undefined}>
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
        ...
        <header className="app-page-header flex flex-col gap-5 py-9 lg:flex-row lg:items-end lg:justify-between">
          <div><h1 className="app-page-title">学学学</h1><p className="app-page-description mt-3 max-w-xl">把考试目标、科目进度和每天的学习记录放在同一个工作台。</p></div>
          <section aria-label="学习操作" className="app-actions grid grid-cols-1 gap-2 sm:grid-cols-3">
            <Button className="min-h-11 gap-2" data-dialog-focus="exam-create" onClick={() => openExamCreate()}><Plus aria-hidden="true" className="h-4 w-4" />新建考试</Button>
            <Button variant="outline" className="min-h-11 gap-2" data-dialog-focus="subject-create" onClick={() => openSubjectCreate()} disabled={!selectedExam}><BookOpen aria-hidden="true" className="h-4 w-4" />新建科目</Button>
            <Button variant="outline" className="min-h-11 gap-2" data-dialog-focus="checkin" onClick={() => openCheckin()} disabled={subjects.length === 0}><CalendarPlus aria-hidden="true" className="h-4 w-4" />学习打卡</Button>
          </section>
        </header>
        ...
        <div className="grid gap-5 lg:grid-cols-[1.25fr_0.75fr]">
          ...
          <section className="learning-panel" aria-label="学习总览"><p className="learning-kicker">学习总览</p><div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-2"><div><p className="app-stat-label">总进度</p><p className="app-stat-value mt-1">{progress?.summary.overallProgressPercentage ?? 0}%</p></div><div><p className="app-stat-label">学习时长</p><p className="app-stat-value mt-1">{progress?.summary.totalStudyHours ?? 0}h</p></div><div><p className="app-stat-label">打卡次数</p><p className="app-stat-value mt-1">{progress?.summary.totalCheckins ?? 0}</p></div><div><p className="app-stat-label">完成科目</p><p className="app-stat-value mt-1">{progress?.summary.completedSubjectsCount ?? 0} / {progress?.summary.subjectsCount ?? subjects.length}</p></div></div></section>
        </div>
        ...
        <section className="learning-panel mt-5" aria-labelledby="recent-learning-title">
          <div className="flex flex-wrap items-end justify-between gap-3 border-b border-border pb-4"><div><p className="learning-kicker">记录管理</p><h2 id="recent-learning-title" className="mt-1 text-xl font-semibold">近期学习记录</h2></div><span className="text-sm text-muted-foreground">显示 {checkins.length} 条</span></div>
