    ...
    <Dialog open={open} onOpenChange={(nextOpen) => !submitting && onOpenChange(nextOpen)}>
      <DialogContent aria-describedby="wedding-task-description" onEscapeKeyDown={(event) => submitting && event.preventDefault()} onPointerDownOutside={(event) => submitting && event.preventDefault()} onInteractOutside={(event) => submitting && event.preventDefault()}>
        ...
        <form className="grid gap-4" onSubmit={handleSubmit} noValidate>
          ...
          <div className="grid gap-2"><Label htmlFor="wedding-task-planned-date">计划日期</Label><Input id="wedding-task-planned-date" type="date" className="min-h-11" value={plannedDate} onChange={(event) => { setPlannedDate(event.target.value); setFieldError('plannedDate', '') }} disabled={submitting} aria-invalid={Boolean(errors.plannedDate)} aria-describedby={errors.plannedDate ? 'wedding-task-planned-date-error' : undefined} />{errors.plannedDate && <p id="wedding-task-planned-date-error" className="text-sm text-destructive">{errors.plannedDate}</p>}</div>
