...
import { actionOwnerOptions, marriageNodeOptions, priorityOptions, taskStatusOptions, weddingCategoryOptions } from './wedding.constants'
...
type Field = 'taskName' | 'category' | 'plannedDate' | 'status' | 'priority' | 'stageKey' | 'ownerRole' | 'completionCriteria' | 'notes'
...
interface WeddingTaskDialogProps {
  ...
  defaultStageKey?: MarriageNodeKey
}
...
export default function WeddingTaskDialog({ open, onOpenChange, onSubmit, task = null, processId = null, defaultStageKey = 'wedding' as MarriageNodeKey }: WeddingTaskDialogProps) {
  const editing = Boolean(task)
  const [taskName, setTaskName] = useState('')
  const [category, setCategory] = useState<WeddingTaskCategory | ''>('')
  const [plannedDate, setPlannedDate] = useState('')
  const [stageKey, setStageKey] = useState<MarriageNodeKey>(defaultStageKey)
  const [ownerRole, setOwnerRole] = useState<ActionOwnerRole>('both' as ActionOwnerRole)
  ...
  useEffect(() => {
    ...
    setStageKey(task?.stageKey ?? defaultStageKey)
    setOwnerRole(task?.ownerRole ?? 'both' as ActionOwnerRole)
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (completionCriteria.trim().length > 500) nextErrors.completionCriteria = '完成标准不能超过 500 个字符'
    ...
    const data: TaskFormData = editing
      ? {
          ...
          processId: processId ?? task?.processId ?? null,
          stageKey,
          ownerRole,
      ...
      : {
          ...
          ...(processId ? { processId, stageKey, ownerRole, completionCriteria: completionCriteria.trim() || null } : {}),
          status,
          priority: Number(priority),
    ...
    <Dialog open={open} onOpenChange={(nextOpen) => !submitting && onOpenChange(nextOpen)}>
      <DialogContent aria-describedby="wedding-task-description" onEscapeKeyDown={(event) => submitting && event.preventDefault()} onPointerDownOutside={(event) => submitting && event.preventDefault()} onInteractOutside={(event) => submitting && event.preventDefault()}>
        ...
        <form className="grid gap-4" onSubmit={handleSubmit} noValidate>
          ...
          <div className="grid gap-2"><Label htmlFor="wedding-task-stage">所属阶段</Label><Select value={stageKey} onValueChange={(value) => { setStageKey(value as MarriageNodeKey); setFieldError('stageKey', '') }} disabled={submitting}><SelectTrigger id="wedding-task-stage" className="min-h-11"><SelectValue /></SelectTrigger><SelectContent>{marriageNodeOptions.map(({ value, label }) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>
          <div className="grid gap-2"><Label htmlFor="wedding-task-owner">负责人</Label><Select value={ownerRole} onValueChange={(value) => { setOwnerRole(value as ActionOwnerRole); setFieldError('ownerRole', '') }} disabled={submitting}><SelectTrigger id="wedding-task-owner" className="min-h-11"><SelectValue /></SelectTrigger><SelectContent>{actionOwnerOptions.map(({ value, label }) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select></div>
          ...
          <div className="grid gap-2"><Label htmlFor="wedding-task-completion-criteria">完成标准</Label><Textarea id="wedding-task-completion-criteria" className="min-h-20" value={completionCriteria} onChange={(event) => { setCompletionCriteria(event.target.value); setFieldError('completionCriteria', '') }} maxLength={500} placeholder="例如：双方确认日期并完成预约" disabled={submitting} aria-invalid={Boolean(errors.completionCriteria)} />{errors.completionCriteria && <p role="alert" className="text-sm text-destructive">{errors.completionCriteria}</p>}</div>
