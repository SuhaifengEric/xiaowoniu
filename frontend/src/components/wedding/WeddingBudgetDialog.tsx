    ...
    <Dialog open={open} onOpenChange={(nextOpen) => !submitting && onOpenChange(nextOpen)}>
      <DialogContent aria-describedby="wedding-budget-description" onEscapeKeyDown={(event) => { if (submitting) { event.preventDefault(); return } event.preventDefault(); onOpenChange(false) }} onPointerDownOutside={(event) => submitting && event.preventDefault()} onInteractOutside={(event) => submitting && event.preventDefault()}>
        ...
        <form className="grid gap-4" onSubmit={handleSubmit} noValidate>
          ...
          <div className="grid gap-2"><Label htmlFor="wedding-budget-date">婚礼日期</Label><DatePicker id="wedding-budget-date" value={weddingDate} onValueChange={(value) => { setWeddingDate(value); setErrors((current) => ({ ...current, weddingDate: undefined })) }} clearLabel="清除婚礼日期" disabled={submitting} aria-invalid={Boolean(errors.weddingDate)} aria-describedby={errors.weddingDate ? 'wedding-budget-date-error' : undefined} />{errors.weddingDate && <p id="wedding-budget-date-error" className="text-sm text-destructive">{errors.weddingDate}</p>}</div>
