...
export function WeddingOverview({ overview, loading, onEditBudget }: WeddingOverviewProps) {
  ...
  return <section className="wedding-panel" aria-labelledby="wedding-overview-title">
    <div className="wedding-overview-header flex flex-wrap items-end justify-between gap-3 border-b border-stone-200 pb-4">
      <div>
    ...
    <div className="wedding-overview-grid mt-4 grid gap-px overflow-hidden rounded-sm border border-stone-200 bg-stone-200 sm:grid-cols-2 lg:grid-cols-4">
      <div className="bg-white p-4"><p className="text-xs font-semibold uppercase text-stone-500">婚期倒计时</p><p className="wedding-overview-value mt-2 flex items-center gap-2 text-2xl font-semibold text-stone-950"><CalendarHeart aria-hidden="true" className="h-5 w-5 shrink-0 text-pink-700" /><span className="break-words">{countdownText(overview?.daysUntilWedding ?? null)}</span></p><p className="mt-1 text-sm text-stone-600">{budget ? `婚礼日期 ${budget.weddingDate}` : '设置婚期后显示倒计时'}</p></div>
      <div className="bg-white p-4"><p className="text-xs font-semibold uppercase text-stone-500">总预算</p><p className="wedding-overview-value mt-2 text-2xl font-semibold text-stone-950">{budget ? money(budget.totalBudget) : '未设置'}</p><p className="mt-1 text-sm text-stone-600">{budget ? `已用 ${usedPercentage ?? 0}%` : '先设置一个预算'}</p></div>
      <div className="bg-white p-4"><p className="text-xs font-semibold uppercase text-stone-500">实际花费</p><p className="wedding-overview-value mt-2 text-2xl font-semibold text-stone-950">{money(overview?.actualExpenseTotal ?? 0)}</p><p className="mt-1 text-sm text-stone-600">{overview?.expenseCount ?? 0} 笔花费</p></div>
      <div className="bg-white p-4"><p className="text-xs font-semibold uppercase text-stone-500">预算余量</p><p className={`wedding-overview-value mt-2 text-2xl font-semibold ${remaining !== null && remaining < 0 ? 'text-red-800' : 'text-stone-950'}`}>{remaining === null ? '—' : money(remaining)}</p><p className={`mt-1 text-sm ${remaining !== null && remaining < 0 ? 'font-medium text-red-800' : 'text-stone-600'}`}>{remaining !== null && remaining < 0 ? `已超支 ${money(Math.abs(remaining))}` : remaining === null ? '没有预算数据' : '可继续安排'}</p></div>
    </div>
    <div className="wedding-overview-secondary mt-4 grid gap-4 sm:grid-cols-2">
      <div className="rounded-sm border border-stone-200 bg-white p-4">
        ...
        <p className="wedding-overview-value mt-2 text-2xl font-semibold text-stone-950">{overview?.taskCounts.completionPercentage ?? 0}%</p>
      ...
      <div className="rounded-sm border border-stone-200 bg-white p-4">
        ...
        <p className="wedding-overview-value mt-2 text-2xl font-semibold text-stone-950">{money(overview?.plannedExpenseTotal ?? 0)}</p>
    ...
    <div className="wedding-overview-breakdown mt-6">
      ...
      <div className="wedding-overview-chart mt-4 min-h-[220px] min-w-0 overflow-hidden" aria-label="类别计划与实际花费对比">
