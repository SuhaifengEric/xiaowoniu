...
export function MarriageAgreementList({ agreements, loading, onCreate, onUpdate, onArchive }: MarriageAgreementListProps) {
  ...
  return <section className="wedding-panel" aria-labelledby="marriage-agreements-title">
    ...
    <div className="mt-4 grid gap-3">{agreements.map((item) => { const draft = drafts[item.id] ?? { title: item.title, notes: item.notes ?? '' }; const editing = editingId === item.id; return <article key={item.id} className={`agreement-row ${item.status === 'needs_discussion' ? 'agreement-row--risk' : ''}`}>
      ...
      <div className="agreement-actions flex shrink-0 flex-col items-end gap-2 sm:flex-row sm:items-center">{editing ? <Button type="button" className="min-h-11 gap-2" onClick={() => void saveEdit(item)} disabled={busy === item.id}><Check aria-hidden="true" className="h-4 w-4" />保存</Button> : <><select aria-label={`议题状态${item.title}`} className="agreement-status min-h-11 rounded-xl border border-input bg-card px-3 text-sm" value={item.status} onChange={(event) => void onUpdate(item.id, { status: event.target.value as AgreementStatus })}>{statuses.map((status) => <option key={status} value={status}>{agreementStatusOptions.find((option) => option.value === status)?.label}</option>)}</select><Button type="button" variant="ghost" className="agreement-action wedding-icon-button" aria-label={`编辑议题${item.title}`} onClick={() => beginEdit(item)}><Pencil aria-hidden="true" className="h-4 w-4" /><span>编辑</span></Button><Button type="button" variant="ghost" className="agreement-action wedding-icon-button" aria-label={`归档议题${item.title}`} onClick={() => window.confirm('归档后该议题将不再出现在当前列表，确定继续吗？') && void onArchive(item.id)}><Archive aria-hidden="true" className="h-4 w-4" /><span>归档</span></Button></>}</div>
    </article> })}</div>
  </section>
}
